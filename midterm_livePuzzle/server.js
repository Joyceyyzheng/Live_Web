...
var options = {
  key: fs.readFileSync("/etc/letsencrypt/live/yz4867.itp.io/privkey.pem"),
  cert: fs.readFileSync("/etc/letsencrypt/live/yz4867.itp.io/fullchain.pem"),
};
...
io.sockets.on(
  ...
  // Listen for mouse position data from clients
  ...
  function (socket) {
    ...
    socket.on("mousePosition", (data) => {
      // Broadcast the mouse position data to all connected
      mousePositions[data.id] = [data.x, data.y];
      // console.log(mousePositions);

      socket.broadcast.emit("mousePosition", mousePositions);
      // console.log("emitting mouse pos");
    });
    ...
    socket.on("draggedPosition", (data) => {
      ...
      socket.broadcast.emit("draggedPosition", data);
      console.log("sending dragged positions");
    });
